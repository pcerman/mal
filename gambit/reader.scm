...
(define (create-reader str/port)
  (if (string? str/port)
    (make-reader (open-input-string "")
                 (void) str/port 0 1)
    (make-reader str/port (void) #f 1 1)))

(define (reader-pos rdr)
  (if (zero? (reader-row rdr))
      (reader-col rdr)
...
(define (next-token rdr)
  (let loop ((token (reader-token rdr)))
    (cond ((void? token)
               (loop (read-token rdr)))
          ((string? token)
...
(define (read-form rdr #!optional (require #f))
  (let ((token (next-token rdr)))
    (cond ((eof-object? token) (if require
                                   (error (sprint* "form is expected at: " (reader-pos rdr)))
                                   token))
          ((equal? token "'")  (list 'quote (read-form rdr #t)))
          ((equal? token "`")  (list 'quasiquote (read-form rdr #t)))
          ((equal? token "~")  (list 'unquote (read-form rdr #t)))
          ((equal? token "~@") (list 'splice-unquote (read-form rdr #t)))
          ((equal? token "@")  (list 'deref (read-form rdr #t)))
          ((equal? token "^")  (let ((meta (read-form rdr #t)))
                                 (list 'with-meta (read-form rdr #t) meta)))
          ...
          (else
               (case (string-ref token 0)
                 ((#\) #\] #\})
                      (error (sprint* "unexpected token '" token "' at: " (reader-pos rdr))))
                 ((#\;) (read-form rdr))
                 ((#\") (read-string token))
