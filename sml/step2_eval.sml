...
type EnvType = Mal.Value SDic.Table
...
fun makeEnv () =
    let val e0 = SDic.empty
        val e1 = SDic.insert e0 ("+", (Mal.Function_1N ("+", makeFun_1N madd (Mal.Int 0))))
        val e2 = SDic.insert e1 ("-", (Mal.Function_1N ("-", makeFun_1N msub (Mal.Int 0))))
        val e3 = SDic.insert e2 ("*", (Mal.Function_1N ("*", makeFun_1N mmul (Mal.Int 1))))
        val e4 = SDic.insert e3 ("/", (Mal.Function_1N ("/", makeFun_1N mdiv (Mal.Int 1))))

        val syms = !Mal.Symbols
    in
...
and eval_ast ast env =
    case ast of
        Mal.Symbol name =>
        (case (SDic.lookup env name) of
             NONE => Core.error ("symbol '" ^ name ^ "' not found")
           | SOME v => v)
      | Mal.List lst =>
