...
fun apply (Mal.Closure (vars, rest, exps, env)) args =
    let val env' = Env.make env
        ...
        fun set_vars [] [] = ()
          | set_vars (var :: vars) (arg :: args) =
                (Env.set env' var arg;
                 set_vars vars args)
          | set_vars [] args =
                (case rest of
                     SOME var => (Env.set env' var (Mal.List args); ())
                   | NONE => Core.error "apply - too much arguments")
          | set_vars vars [] = Core.error "apply - not enought arguments"
    in
        set_vars vars args;
